  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        <InputField label="Blood Glucose" unit="mgs/dl" name="bgr" placeholder="e.g. 121" error={errors.bgr?.message as string} />
        <InputField label="Blood Urea" unit="mgs/dl" name="bu" placeholder="e.g. 36" error={errors.bu?.message as string} />
        <InputField label="Serum Creatinine" unit="mgs/dl" name="sc" placeholder="e.g. 1.2" error={errors.sc?.message as string} />
        <InputField label="Sodium" unit="mEq/L" name="sod" placeholder="e.g. 138" error={errors.sod?.message as string} />
        <InputField label="Potassium" unit="mEq/L" name="pot" placeholder="e.g. 4.4" error={errors.pot?.message as string} />
        <InputField label="WBC Count" unit="cells/cumm" name="wbcc" placeholder="e.g. 7800" error={errors.wbcc?.message as string} />
        <InputField label="RBC Count" unit="millions/cmm" name="rbcc" placeholder="e.g. 5.2" error={errors.rbcc?.message as string} />
      </div>
    </div>
  );
};
